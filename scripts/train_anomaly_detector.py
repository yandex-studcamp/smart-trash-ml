...
from src.data.anomaly_dataset import AnomalyDetectionDataset
from src.testers.anomaly_evaluator import AnomalyEvaluator
...
def plot_training_history(history: dict[str, list[float]], output_path: str) -> None:
    ...
    plt.plot(history["train_feature_loss"], label="train_feature_loss", linestyle="--")
    plt.plot(history["val_feature_loss"], label="val_feature_loss", linestyle="--")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Anomaly detector training history")
    plt.legend()
    plt.grid(True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train the reverse-distillation anomaly detector.",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    print(f"=== Anomaly detector training: {args.exp_name} ===")
    ...
    model = cfg.get_model()
    trainer = AnomalyTrainer(config=cfg, save_dir=weights_dir)
    ...
    val_predictions = evaluator.predict(model=model, dataloader=val_loader, save_anomaly_maps=False)

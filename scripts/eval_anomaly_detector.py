...
def build_dataloader(dataset: AnomalyDetectionDataset, batch_size: int, num_workers: int) -> DataLoader:
    return DataLoader(
        dataset,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate the reverse-distillation anomaly detector.",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Weights filename inside experiments/<exp_name>/weights/.",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        "--save_maps",
        action="store_true",
        help="Save anomaly maps for samples from the selected split.",
    )
    ...
    maps_dir = os.path.join(artifacts_dir, f"{args.split}_anomaly_maps")
    ...
    print(f"=== Anomaly detector evaluation: {args.exp_name} / {args.split} ===")
